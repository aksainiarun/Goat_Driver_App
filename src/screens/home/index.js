import { Text, StyleSheet, View, ScrollView, Image, FlatList, Animated, Easing, Platform } from 'react-native'
...
class Home extends Component {
    ...
    handleFreeSpace = (height) => {
        if (height < SCREEN_HEIGHT / 2) {
            this.setState({
                // freeHeight: (SCREEN_HEIGHT - height)
                // (insets.bottom + insets.top)
                // Platform.select({ ios: insets.top, android: 17 })
            })
        }
    ...
    render() {
        ...
        const { isSnapEnd, selectedTab, freeHeight } = this.state
        return (
            <Container>
                ...
                >
                    ...
                    <View style={{ zIndex: 99, backgroundColor: BACKGROUND_COLOR, borderTopRightRadius: 25, borderTopLeftRadius: 25, marginTop: Platform.select({ ios: -190, android: -100 }), }} onLayout={(e) => this.handleFreeSpace(e.nativeEvent.layout.height)}>
